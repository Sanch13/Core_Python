...
def main():
    ...
    c = d = [2, 4, 6]   # [2, 4, 6]
    """Эти элементы списка (c[0], c[1] и c[2]) сами по себе являются именами, указывающими на 
    целочисленные объекты со значениями 2, 4 и 6. Список хранит элементы в заданном порядке. 
    Теперь давайте изменим первый элемент списка с помощью имени c и убедимся, что список d 
    также изменился. Когда первый элемент списка изменяется, он больше не указывает на объект 
    со значением 2. Теперь он указывает на объект со значением 99. Список все еще имеет тип list,
